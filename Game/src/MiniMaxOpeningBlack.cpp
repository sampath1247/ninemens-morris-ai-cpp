...
// Find the best move for Black in the opening phase
Board findBestMoveBlack(const Board &board, int depth) {
    int bestScore = INT_MAX; // Black minimizes the score
    Board bestMove = board;
    vector<Board> moves = generateMovesOpening(board, 'B'); // Moves for Black
    ...
    for (const auto &m : moves) {
        int score = minimax(m, depth - 1, true); // White's turn next
        if (score < bestScore) {
            bestScore = score;
            bestMove = m;
        }
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        cerr << "Usage: MiniMaxOpeningBlack <infile> <outfile> <depth>" << endl;
        return 1;
    }
