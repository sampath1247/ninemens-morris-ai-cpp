...
// Generate all possible removals after a mill
void GenerateRemove(const Board &board, vector<Board> &L, char opponent) {
    ...
    if (!removed) { // All in mills, remove any piece
        for (int i = 0; i < 23; ++i) {
            if (board.getPosition(i) == opponent) {
                Board b2 = board;
                b2.setPosition(i, 'x');
                L.push_back(b2);
                break; // Only one removal needed
            }
        }
    }
}

// Generate moves for midgame/endgame (no hopping)
...
// Generate hopping moves (endgame)
...
int evaluate(const Board &board) {
    positionsEvaluated++;
    int numWhite = countPieces(board, 'W');
    int numBlack = countPieces(board, 'B');
    if (numBlack <= 2) return 10000;  // White wins
    if (numWhite <= 2) return -10000; // Black wins
    vector<Board> blackMoves = generateMovesMidgameEndgame(board, 'B');
    if (blackMoves.empty()) return 10000;   // White wins
    vector<Board> whiteMoves = generateMovesMidgameEndgame(board, 'W');
    if (whiteMoves.empty()) return -10000;  // Black wins
    return 1000 * (numWhite - numBlack) - blackMoves.size();
}

// Sort moves to prioritize mill-forming moves
void sortMoves(vector<Board> &moves, bool isWhiteTurn) {
    sort(moves.begin(), moves.end(), [isWhiteTurn](const Board &a, const Board &b) {
        bool aFormsMill = false, bFormsMill = false;
        for (int i = 0; i < 23; ++i) {
            if (a.getPosition(i) != 'x' && closeMill(i, a)) aFormsMill = true;
            if (b.getPosition(i) != 'x' && closeMill(i, b)) bFormsMill = true;
        }
        return aFormsMill > bFormsMill;
    });
}

// Alpha-Beta pruning
int alphaBeta(const Board &board, int depth, bool isWhiteTurn, int alpha, int beta) {
    ...
    sortMoves(moves, isWhiteTurn);

    if (isWhiteTurn) {
        int maxEval = INT_MIN;
        for (const Board &move : moves) {
            int eval = alphaBeta(move, depth - 1, false, alpha, beta);
        ...
        int minEval = INT_MAX;
        for (const Board &move : moves) {
            int eval = alphaBeta(move, depth - 1, true, alpha, beta);
...
Board findBestMove(const Board &board, int depth) {
    ...
    for (const Board &move : moves) {
        int score = alphaBeta(move, depth - 1, false, INT_MIN, INT_MAX);
        if (score > bestScore) {
            bestScore = score;
            bestMove = move;
        }
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        cerr << "Usage: " << argv[0] << " <input_file> <output_file> <depth>" << endl;
        return 1;
    }
