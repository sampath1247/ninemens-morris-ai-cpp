...
static const vector<vector<int>> neighbors = {
    ...
    {14, 18, 20}, // 17: b5 -> c4, d5, a6
    {15, 17, 19, 21}, // 18: d5 -> d4, b5, f5, d6
    {12, 16, 18}, // 19: f5 -> f3, e4, d5
...
Board findBestMoveBlack(const Board &board, int depth) {
    int bestScore = INT_MAX; // Black minimizes the score
    Board bestMove = board;
    vector<Board> moves = GenerateMovesMidgameEndgame(board, 'B'); // Moves for Black
    if (moves.empty()) {
        cerr << "No moves available for Black!" << endl;
        return board;
    }
    for (const auto &m : moves) {
        int score = minimaxMidgameEndgame(m, depth - 1, true); // White's turn next
        if (score < bestScore) {
            bestScore = score;
            bestMove = m;
        }
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        cerr << "Usage: MiniMaxGameBlack <infile> <outfile> <depth>\n";
        return 1;
    }
    string inF = argv[1];
    string outF = argv[2];
    ...
    Board best = findBestMoveBlack(board, depth);
